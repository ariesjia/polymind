...
import { parseThinkBlocks, normalizeMarkdownTables } from "@/lib/utils/aiContent";
...
import type { AIToolEvent } from "@/lib/api/ai";
import type { AIConfig, PolyEvent, Market } from "@/lib/types";
...
export function useAIAnalysis(
  ...
  const runAnalysis = useCallback(() => {
    ...
    if (token) {
      ...
      import("@/lib/api/ai-client").then(({ streamAnalysisFromAPI }) =>
        streamAnalysisFromAPI(
          token,
          event,
          markets,
          (chunk) => {
    ...
    setError("Please connect your wallet to run AI analysis.");
    setStarted(true);
  }, [config, event, markets, onComplete, token]);
...
interface AITriggerBarUser {
  credits: number;
  hasTavilyKey: boolean;
}

export function AITriggerBar({
  ai,
  aiConfig,
  token,
  user,
  ...
  aiConfig: AIConfig;
  token?: string | null;
  user?: AITriggerBarUser | null;
}) {
  const [confirmOpen, setConfirmOpen] = useState(false);
  const missingConfig = !token && (!aiConfig.apiKey || !aiConfig.baseUrl);
  const insufficientCredits = !!token && (user?.credits ?? 0) < 1;
  const isDisabled = ai.loading || missingConfig || insufficientCredits;
    ...
    <div className="shrink-0 border-t border-white/[0.06] bg-[#12141d]/90 px-7 py-4 backdrop-blur-md">
      {token && !user?.hasTavilyKey && (
        <p className="mb-3 rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-2 text-xs text-amber-400">
          ...
          >
            Configure in Member Center &gt; Settings
          </Link>{" "}
          for AI web search during analysis.
        </p>
      )}
      <button
        onClick={handleClick}
      ...
      {missingConfig && (
        <p className="mt-2 text-center text-xs text-zinc-500">
          Please connect your wallet to run AI analysis.
        </p>
      )}
      {insufficientCredits && (
        <p className="mt-2 text-center text-xs text-zinc-500">
            ...
            {confirmOpen && (
              <>
                ...
                <motion.div
                  ...
                  className="fixed left-1/2 top-1/2 z-[100] w-[calc(100vw-2rem)] max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-white/[0.08] bg-[#16182a] p-6 shadow-2xl"
                >
                  <h3 className="text-lg font-bold text-white">Confirm AI Analysis</h3>
                  <p className="mt-2 text-sm text-zinc-400">
                    This will consume 1 credit. Continue?
                  </p>
                  <div className="mt-6 flex gap-3">
                    <button
                      onClick={() => setConfirmOpen(false)}
                      className="flex-1 rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-2.5 text-sm font-medium text-zinc-300 transition-colors hover:bg-white/[0.08]"
                    >
                      Cancel
                    </button>
                    <button
                      onClick={handleConfirm}
                      className="flex-1 rounded-xl bg-indigo-500 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-indigo-600"
                    >
                      Confirm
                    </button>
                  </div>
